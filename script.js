function safeRun(fn){
  try { fn(); } catch(err) { console.error('某个模块出错，已跳过：', err); }
}

// ---------- boot sequence ----------
// 不再等 window.load（不会因为图片没下完就卡死）
safeRun(() => {
...
safeRun(() => {
  ...
  if(!shootArena || !shootScoreEl || !shootTimeEl || !shootStartBtn || !shootResult) return;
  ...
  function spawnTarget(){
    ...
    const missTimer = setTimeout(()=>{ if(target.parentNode) target.remove(); }, lifetime);
